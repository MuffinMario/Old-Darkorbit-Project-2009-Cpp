...
public:
	...
	// X and Y are needed for inRadiationzone. Always ... static std::string events(bool inDemizone, bool playRepanim, bool inTradezone, bool inRadiationzone, bool inJumpzone, bool repvouchs);
	...
	static std::string initDronesVisual(id_t id, std::string visualString);
	static std::string initDronesInformational(id_t id,size_t flax, size_t iris);
	...
	static std::string collectOre(id_t oreid);
