...
std::string CPacketComposer::initDronesVisual(id_t id,std::string visualString)
{
	CPacketer packet(SETDRONES);
	packet.push(to_string(id));
	return packet.pushAndGet(visualString);
}

std::string CPacketComposer::initDronesInformational(id_t id,size_t flax, size_t iris)
{
	CPacketer packet(SETDRONESALT);
	packet.push(to_string(id));
	std::stringstream ss; ss << flax << "/" << iris;
	return packet.pushAndGet(ss.str());
}
...
std::string CPacketComposer::init(id_t userID, username_t username, shipid_t shipID, speed_t speed, shield_t shield, 
	...
	map_t mapID, factionid_t factionid, clanid_t clanid, ammo_t battmax, rocket_t rocketsmax, weaponstate_t oState, 
	...
	jackpot_t jackpot, rank_t userrank, clan_t clantag, gates_t gatesAchieved, bool useSysFont)
{
	...
	std::deque<std::string> information = generateStringDeque(userID, username, shipID, speed, shield, shieldmax, health, healthmax, cargospace, cargospacemax, x, y, mapID, factionid, clanid, battmax, rocketsmax,
...
std::string CPacketComposer::collectOre(id_t oreid)
{
	CPacketer packet(COLLECTORE);
	return packet.pushAndGet(to_string(oreid));
}
...
std::string CPacketComposer::peaceArea()
{
	CPacketer packet(PEACEAREA);
	return packet;
}
