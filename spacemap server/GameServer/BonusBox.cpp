...
void CBonusBox::rewardPlayer(id_t playerid)
{
	...
	if (m_bb_content.m_ammo.m_ammo_amount != 0)
	{
		...
		ammo_t amount = static_cast<ammo_t>(m_bb_content.m_ammo.m_ammo_amount * multiplier);
		if (ammoType != "")
		{
	...
	{
		if (rewards++ > 0) sqlSET += ", ";
		long long plus = static_cast<long long>(m_bb_content.m_credits * multiplier);
	...
	{
		if (rewards++ > 0) sqlSET += ", ";
		long long plus = static_cast<long long>(m_bb_content.m_uridium * multiplier);
	...
	{
		if (rewards++ > 0) sqlSET += ", ";
		float plus = static_cast<float>(m_bb_content.m_jackpot *multiplier);
	...
	{
		if(rewards++ > 0) sqlSET += ", ";
		extraenergy_t energy = static_cast<extraenergy_t>(m_bb_content.m_extraenergy * multiplier);
...
std::shared_ptr<CBonusBox> CBonusBox::generateNewBonusBox(id_t id, map_t mapid,std::reference_wrapper<CSession> session)
{
	...
	if (mapid != 42)
	{
		...
		lt = static_cast<CBonusBox::loottype_t>(random<int>(max)); // re-cast because integer type
		...
		case CBonusBox::loottype_t::LT_CRED:
			amount = static_cast<double>(cr_p.at(random<size_t>(cr_p.size() - 1)));
			break;
		case CBonusBox::loottype_t::LT_URI:
			amount = static_cast<double>(u_p.at(random<size_t>(u_p.size() - 1)));
			break;
		case CBonusBox::loottype_t::LT_AMMO_X1:
			amount = static_cast<double>(aax1_p.at(random<size_t>(aax1_p.size() - 1)));
			break;
		case CBonusBox::loottype_t::LT_AMMO_X2:
		case CBonusBox::loottype_t::LT_AMMO_X3:
		case CBonusBox::loottype_t::LT_AMMO_SAB:
			amount = static_cast<double>(aax23SAB_p.at(random<size_t>(aax23SAB_p.size() - 1)));
			break;
		case CBonusBox::loottype_t::LT_JP:
			amount = static_cast<double>(jp_p.at(random<size_t>(jp_p.size() - 1)));
			break;
		case CBonusBox::loottype_t::LT_EE:
			amount = static_cast<double>(ee_p.at(random<size_t>(ee_p.size() - 1)));
			break;
		default:
