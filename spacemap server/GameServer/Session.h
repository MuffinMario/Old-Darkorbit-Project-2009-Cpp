...
public:
	...
	typedef std::unordered_map<id_t, handlePtr> ConnectionContainer_t; // identifiable by id
...
public:
	explicit CSession(map_t mapid);
	~CSession() = default;
	CSession(const CSession&) = default;
	...
	void foreachCollectable(std::function<void(id_t, std::shared_ptr<ICollectable>&)> f);
	void foreachMobs(std::function<void(id_t, std::shared_ptr<CMob>&)> f);
	void foreachConnections(std::function<void(id_t, handlePtr&)> f);

	void foreachCollectable(std::function<void(id_t, std::shared_ptr<ICollectable>&)> f, std::function<bool(id_t, std::shared_ptr<ICollectable>&)> pred);
	void foreachMobs(std::function<void(id_t, std::shared_ptr<CMob>&)> f, std::function<bool(id_t, std::shared_ptr<CMob>&)> pred);
	void foreachConnections(std::function<void(id_t, handlePtr&)> f, std::function<bool(id_t, handlePtr&)> pred);
