...
CPacketer::CPacketer(std::string operationHeader) {
...
void CPacketer::push(std::string cmd)
{
	m_savedPacket.push_back("|" + cmd );
}

void CPacketer::push(std::deque<std::string> i)
{
	for (auto& it : i) {
		push(it);
	}
...
CPacketer CPacketer::pushAndGet(std::string cmd)
{
	push(cmd);
	return *this;
}

CPacketer CPacketer::pushAndGet(std::deque<std::string> i)
{
	push(i);
	return *this;
}
