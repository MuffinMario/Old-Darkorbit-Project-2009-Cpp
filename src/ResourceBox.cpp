...
void CResourceBox::spawn()
{
	if (boxIsPrivate())
	{
		...
		// beautiful 
		m_session.get().sendEveryoneBut(createBlue, 
			(std::function<bool(handlePtr)>)std::bind(&CResourceBox::userGetsBlueHandle,this, std::placeholders::_1));
		m_session.get().sendEveryoneBut(createOrange, 
			(std::function<bool(handlePtr)>)std::bind(&CResourceBox::userGetsOrangeHandle, this, std::placeholders::_1));
	}
	else
	{
		std::string createOrange = m_pm.createLoot(m_id, RESOURCE_BOX_DEFAULT_COLLECTABLE_ID, m_pos.first, m_pos.second);
